...
from transformer.modern_transformer import Transformer
...
model = Transformer(SRC_VOCAB, TGT_VOCAB, D_MODEL, NUM_LAYERS,
                    NUM_HEADS, NUM_KV_HEADS, NUM_EXPERTS,
                    TOPK, 0.1, MAX_LEN, PAD_ID)
...
for epoch in range(NUM_EPOCHS):
    logits, aux_loss = model(src_indices, tgt_indices)
    loss = criterion(logits, targets) + aux_loss  # 添加辅助损失
